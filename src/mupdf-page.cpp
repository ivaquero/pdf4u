...
static void clear_bgr_samples_with_value(unsigned char *samples, int size,
...
QPointF mapToOrigin(const QPointF &pos, float scaleX, float scaleY,
                    float rotation) {
...
QSizeF mapToOrigin(const QSizeF &size, float scaleX, float scaleY,
                   float rotation) {
  fz_matrix transform = fz_identity;
  fz_matrix inverse;
  fz_point vector;
...
QRectF mapToOrigin(const QRectF &rect, float scaleX, float scaleY,
                   float rotation) {
  fz_matrix transform = fz_identity;
  fz_matrix inverse;
  fz_rect r;
...
QPointF mapFromOrigin(const QPointF &pos, float scaleX, float scaleY,
                      float rotation) {
  fz_matrix transform = fz_identity;
  fz_point point;
...
QSizeF mapFromOrigin(const QSizeF &size, float scaleX, float scaleY,
                     float rotation) {
  fz_matrix transform = fz_identity;
  fz_point vector;
...
QRectF mapFromOrigin(const QRectF &rect, float scaleX, float scaleY,
                     float rotation) {
  fz_matrix transform = fz_identity;
  fz_rect r;
...
PagePrivate::PagePrivate(DocumentPrivate *dp, int index)
    : documentp(dp), context(documentp->context), document(documentp->document),
      page(NULL), display_list(NULL), text_sheet(NULL), text_page(NULL),
      transparent(documentp->transparent), b(documentp->b), g(documentp->g),
      r(documentp->r), a(documentp->a) {
  fz_try(context) {
    fz_rect bounds;
 ...
 */
bool Page::isValid() const { return (d && d->page) ? true : false; }
...
QImage Page::renderImage(float scaleX, float scaleY, float rotation) const {
  ...
  fz_try(d->context) {
    ...
    pixmap =
        fz_new_pixmap_with_bbox(d->context, fz_device_bgr(d->context), &bbox);
#else
    // use rgba for Qt5.2
    pixmap =
        fz_new_pixmap_with_bbox(d->context, fz_device_rgb(d->context), &bbox);
#endif
 ...
 */
void Page::setTransparentRendering(bool enable) { d->transparent = enable; }
