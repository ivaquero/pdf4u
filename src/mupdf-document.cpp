...
Document *loadDocument(const QByteArray &bytes) {
  ...
  // Create DocumentPrivate
  documentp =
      new DocumentPrivate((unsigned char *)bytes.data(), bytes.length());
  if (!documentp)
    return NULL;
  else if (!documentp->context || !documentp->document) {
    delete documentp;
...
DocumentPrivate::DocumentPrivate(const QString &filePath)
    : context(NULL), document(NULL), transparent(false), b(-1), g(-1), r(-1),
      a(-1) {
  // create context
 ...
 */
bool Document::needsPassword() const { return fz_needs_password(d->document); }
...
bool Document::authPassword(const QString &password) {
  return fz_authenticate_password(d->document, password.toLocal8Bit().data());
}
...
int Document::numPages() const {
  int ret;
  fz_try(d->context) { ret = fz_count_pages(d->document); }
  fz_catch(d->context) { ret = -1; }
  return ret;
}
 ...
 * @brief PDF title
 */
QString Document::title() const { return d->info("Title"); }

/**
 * @brief Author of the document
 */
QString Document::author() const { return d->info("Author"); }

/**
 * @brief The subject of the document
 */
QString Document::subject() const { return d->info("Subject"); }

/**
 * @brief Keywords associated with the document
 */
QString Document::keywords() const { return d->info("Keywords"); }

/**
 * @brief If the document was converted to PDF from another format, the name of
 * the application (for example, Adobe FrameMaker®) that created the original
 * document from which it was converted
 */
QString Document::creator() const { return d->info("Creator"); }

/**
 * @brief If the document was converted to PDF from another format, the name of
 * the application (for example, Acrobat Distiller) that converted it to PDF
 */
QString Document::producer() const { return d->info("Producer"); }
...
QDateTime Document::creationDate() const {
  ...
  return QDateTime::fromString(str.left(16), "'D:'yyyyMMddHHmmss");
}
 ...
 */
void Document::setTransparentRendering(bool enable) { d->transparent = enable; }
