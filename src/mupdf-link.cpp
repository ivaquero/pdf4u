 ...
 */
int LinkGoto::page() const { return d->dest.ld.gotor.page; }
 ...
 */
QString LinkURI::uri() const { return d->dest.ld.uri.uri; }

/**
 * If true, the x and y coords (as ints, in user
 * space) should be appended to the URI before launch.
 */
bool LinkURI::isMap() const { return d->dest.ld.uri.is_map; }

/**
 * @brief A UTF-8 file specification to launch.
 */
QString LinkLaunch::file() const { return d->dest.ld.launch.file_spec; }

/**
 * @brief If true, the destination should be launched in a new window.
 */
bool LinkLaunch::newWindow() const { return d->dest.ld.launch.new_window; }

/**
 * @brief If true, file() is a URI to launch.
 */
bool LinkLaunch::isURI() const { return d->dest.ld.launch.is_uri; }

/**
 * @brief The named action to perform.
 */
QString LinkNamed::named() const { return d->dest.ld.named.named; }
 ...
 */
int LinkGotoR::page() const { return d->dest.ld.gotor.page; }
 ...
 * @return Maybe empty.
 */
QString LinkGotoR::destination() const { return d->dest.ld.gotor.dest; }

/**
 * @brief A pointer to a remote file specification (UTF-8).
 * If set, this destination should cause a new file to be opened.
 */
QString LinkGotoR::file() const { return d->dest.ld.gotor.file_spec; }

/**
 * @brief If true, the destination should open in a new window.
 */
bool LinkGotoR::newWindow() const { return d->dest.ld.gotor.new_window; }
